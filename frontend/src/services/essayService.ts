...
export const generatePlan = async (data: {
  ...
  const res = await $api.post("/v1/essays/plan/generate", data);
  return res.data as { essay_id: number };
};

/* 2. Получить главы по эссе */
export const getChapters = async (essay_id: number) => {
  const res = await $api.get(`/v1/essays/${essay_id}/chapters`);
  return res.data as {
...
export const updateChapterTitle = async (chapter_id: number, title: string) => {
  await $api.put("/v1/essays/chapter/title/update", {
    chapter_id,
    title,
  });
};
...
export const generateEssay = async (essay_id: number) => {
  const res = await $api.post(`/v1/essays/${essay_id}/generate`);
  return res.data;
};

/* 5. Статус генерации */
export const getEssayStatus = async (essay_id: number) => {
  const res = await $api.get(`/v1/essays/${essay_id}/status`);
  return res.data as { status: string };
};

/* 6. Скачать DOCX */
export const downloadEssay = async (essay_id: number) => {
  const res = await $api.get(`/v1/refprint/${essay_id}`, {
    responseType: "blob",
  });
  return res.data;
};
...
export const getAllEssays = async () => {
  const res = await $api.get("/v1/essays");
  return res.data as {
    essays: {
      id: number;
      topic: string;
